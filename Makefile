...
ENV_FILES := --env-file frontend/.env --env-file frontend/.env.local --env-file backend/.env --env-file backend/.env.local
...
.PHONY: help check-env network config build up down restart ps logs frontend-logs backend-logs database-logs frontend-shell backend-shell database-shell clean prod-config prod-build prod-up prod-deploy prod-down prod-restart prod-ps prod-logs
