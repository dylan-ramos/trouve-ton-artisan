...
Sur le domaine final, vérifier HTTPS, redirection, absence de contenu mixte, pages profondes, API, recherche, 404, canonical, robots et sitemap. La route inconnue de cette SPA renvoie le document HTML avec HTTP 200, puis affiche la 404 React et `noindex` ; les ressources API inconnues renvoient HTTP 404. Définir `SMTP_TEST_RECIPIENT` dans `backend/.env.local` avec une boîte contrôlée, puis recréer le backend avec `make prod-up`. Tous les destinataires sont remplacés, sans copie aux adresses du jeu de données. Effectuer un envoi avec un objet identifiable et confirmer la réception ainsi que `Reply-To` ; une réponse HTTP 202 seule ne prouve pas la livraison. Conserver cette redirection pour les démonstrations.
